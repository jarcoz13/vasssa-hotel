...
/*  Created On : 24-feb.-2021 12:30:11 a. m. 				*/
/*  DBMS       : PostgreSQL 						*/
/* ---------------------------------------------------- */
...
(
	...
	q_telefono varchar(20) NOT NULL,	-- numero de telefono del huesped que se registra en eel hortel
...
(
	...
	q_stock integer NOT NULL,	-- Carga el Stock del producto 
	n_nombre_producto varchar(30) NULL	-- Nombre del producto
)
;
...
(
	...
	k_id_persona_tipo smallint NULL,
...
(
	...
	v_descuento numeric(4,2) NOT NULL	-- valor  entero que se calcular
)
;
...
ALTER TABLE cuenta ADD CONSTRAINT "ck_noNegid_cuenta" CHECK (0<=k_id_cuenta)
;
...
ALTER TABLE habitacion ADD CONSTRAINT "ck_noNegid_habitacion" CHECK (0<=k_id_habitacion)
;

ALTER TABLE habitacion ADD CONSTRAINT "ck_noNegnum_camas" CHECK (0<=q_num_camas )
;
...
ALTER TABLE huesped ADD CONSTRAINT "ck_noNegEdad" CHECK (0<q_edad)
;

ALTER TABLE huesped ADD CONSTRAINT "ck_fechaNacimiento" CHECK (extract(year from f_fecha_nacimiento)<extract(year from current_date))
;
...
ALTER TABLE pago ADD CONSTRAINT "ck_noNegid_pago" CHECK (0<=k_id_pago)
;

ALTER TABLE pago ADD CONSTRAINT "ck_noNegmetodo" CHECK (0<=n_metodo)
;
...
ALTER TABLE persona ADD CONSTRAINT "ck_noNegidtipo" CHECK (0<k_id_persona_tipo)
;

ALTER TABLE persona ADD CONSTRAINT "ck_noNegidnumero" CHECK (0<k_id_persona_numero)
;
...
ALTER TABLE producto ADD CONSTRAINT "ck_noNegcodigo" CHECK (0<=k_codigo )
;

ALTER TABLE producto ADD CONSTRAINT "cknoNegprecio" CHECK (0<=v_precio )
;

ALTER TABLE producto ADD CONSTRAINT "ck_noNegstock" CHECK (0<=q_stock)
;
...
ALTER TABLE registro ADD CONSTRAINT "ck_noNegid_registro" CHECK (0<=k_id_Registro)
;

ALTER TABLE registro ADD CONSTRAINT ck_ahora_checkin CHECK (current_date=f_checkin)
;

ALTER TABLE registro ADD CONSTRAINT ck_checkout_sup_checkin CHECK (f_checkin<f_checkout)
;
...
ALTER TABLE reserva ADD CONSTRAINT "ck_noNegid_reserva" CHECK (0<=k_id_reserva)
;

ALTER TABLE reserva ADD CONSTRAINT "ck_noNegnumDias" CHECK (0<=q_num_dias)
;

ALTER TABLE reserva ADD CONSTRAINT "ck_noNegestado" CHECK (0<=i_estado)
;

ALTER TABLE reserva ADD CONSTRAINT ck_intervalo_fecha CHECK (f_inicio<=f_final)
;

ALTER TABLE reserva ADD CONSTRAINT ck_f_inicial_futura CHECK (current_date<=f_inicio)
;
...
ALTER TABLE tipo_habitacion ADD CONSTRAINT "ck_noNegid_tipo_habitacion" CHECK (0<=k_id_tipo_habitacion)
;

ALTER TABLE tipo_habitacion ADD CONSTRAINT "ck_noNegprecio" CHECK (0<=v_precio)
;

ALTER TABLE tipo_habitacion ADD CONSTRAINT "ck_noNegdescuento" CHECK (0<=v_descuento)
;
...
ALTER TABLE registro ADD CONSTRAINT "FK_Registro_huesped"
	FOREIGN KEY (k_id_persona_tipo,k_id_persona_numero) REFERENCES huesped (k_id_persona_tipo,"k_id_persona_ numero") ON DELETE No Action ON UPDATE No Action
;
...
COMMENT ON COLUMN producto.q_stock
	IS 'Carga el Stock del producto '
;

COMMENT ON COLUMN producto.n_nombre_producto
